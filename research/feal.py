...
if __name__ == "__main__":
    import sys
    sys.path.insert(0, ".")
    from emu_auth import feal_cipher as native

    vectors = [
        ("AA:BB:CC:DD:EE:FF", 0x12345678), ("AA:BB:CC:DD:EE:FF", 0x00000000),
        ("AA:BB:CC:DD:EE:FF", 0xFFFFFFFF), ("00:00:00:00:00:00", 0x00000000),
        ("01:02:03:04:05:06", 0xDEADBEEF), ("C0:1A:FF:12:34:56", 0x0000002A),
        ("12:34:56:78:9A:BC", 0x11223344), ("FF:EE:DD:CC:BB:AA", 0x0BADF00D),
        ("A4:C1:38:11:22:33", 0x7FFFFFFF), ("00:11:22:33:44:55", 0x80000000),
    ]
    ok = True
    for mac, v in vectors:
        m = bytes(int(x, 16) for x in mac.split(":"))
        got, exp = feal_cipher(m, v), native(m, v)
        match = got == exp
        ok &= match
        print(f"{'OK  ' if match else 'FAIL'} mac={mac} rnd=0x{v:08X} "
              f"python=0x{got:08X} native=0x{exp:08X}")

    # randomised differential test
    import random
    random.seed(1)
    for _ in range(3000):
        m = bytes(random.randrange(256) for _ in range(6))
        v = random.randrange(1 << 32)
        if feal_cipher(m, v) != native(m, v):
            ok = False
            print("MISMATCH", m.hex(), hex(v))
            break
    print("\nALL VECTORS MATCH" if ok else "\nMISMATCH FOUND")
